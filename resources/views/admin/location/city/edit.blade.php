        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="m-t-50" style="">
                    ...
                    <form action="{{ route('admin.location.city.update', ['city' => $city->id]) }}" method="post" enctype="multipart/form-data" id="city_update">
                        @csrf
                        @method('PUT')
                                ...
                                <select id="state" name="state" class="form-control inputype">
                                    ...
                                    @if ($state)
                                        @foreach ($state as $data)
                                            <option value="{{ $data->id }}" {{ $city->state_id == $data->id ? 'selected' : '' }}>
                                                {{ $data->name }}
                                            </option>
                                        @endforeach
                                    @endif
