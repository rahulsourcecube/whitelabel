        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="m-t-50" style="">
                    <form id="taskadd" method="POST" action="{{ route('company.campaign.update', $task->id) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label for="title">Task Titles <span class="error">*</span></label>
                                <input type="text" class="form-control" id="title" name="title" placeholder="Task Title" maxlength="150"
                                    value="{{ $task->title ?? '' }}">
                                @error('title')
                                ...
                                <div class="int-reward w-80" @if ($task->text_reward) style="display: none;" @endif>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" id="reward" name="reward" placeholder="Reward"
                                            onkeypress="return event.charCode >= 48 && event.charCode <= 57" value="{{ $task->reward ?? '' }}" min="1">
                                    </div>
                                    @error('reward')
                            ...
                            <div class="col-md-6 mt-4">
                                ...
                                <input type="checkbox" name="public" id="public" value="1"{{ $task->public == 1 ? ' checked' : '' }}>
                            </div>
                        ...
                        <div class="custom-reward-text w-80" @if (!$task->text_reward) style="display: none;" @endif>
                            ...
                            <input type="text" name="text_reward" class="form-control" id="text_reward" maxlength="250" value="{{ $task->text_reward ?? '' }}"
                                required>
                            @error('text_reward')
                ...
                @if (isset($type) && $type == '1')
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" class="form-control" id="no_of_referral_users" name="no_of_referral_users" placeholder="No of referral users"
                            onkeypress="return event.charCode >= 48 && event.charCode <= 57" value="{{ $task->no_of_referral_users ?? '0' }}">
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" class="form-control" id="expiry_date" name="expiry_date" placeholder="No Of Task"
                        onkeypress="return event.charCode >= 48 && event.charCode <= 57" value="{{ $task->expiry_date ?? '' }}"
                        min="{{ date($task->expiry_date) }}">
                    @error('expiry_date')
            ...
            <div class="form-row">
                <div class="form-group col-md-3" style="max-height: 200px;">
                    @if (isset($task) && !empty($task->image) && file_exists(base_path('uploads/company/campaign/' . $task->image)))
                        <img id="imagePreview" src="{{ asset('uploads/company/campaign/' . $task->image) }}" alt="Image Preview"
